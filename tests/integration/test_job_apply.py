...
@pytest.mark.asyncio
async def test_process_job_url_match(monkeypatch) -> None:
...
@pytest.mark.asyncio
async def test_apply_to_job_generic(monkeypatch) -> None:
    profile = UserProfile(full_name="Test User", email="test@example.com")
    job = Job(url="https://example.com/job", title="Engineer", company="Acme")
    ...
    result = await service.apply_to_job(job, session_id="sess-2")

    assert result.success
    assert result.method == "form_fill"
